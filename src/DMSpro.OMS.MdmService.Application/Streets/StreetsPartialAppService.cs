...
namespace DMSpro.OMS.MdmService.Streets
{
	public partial class StreetsAppService
	{
		...
		{
			var items = await _streetRepository.GetQueryableAsync();
			var base_dataloadoption = new DataSourceLoadOptionsBase();
			DataLoadParser.Parse(base_dataloadoption,inputDev);
			LoadResult results = DataSourceLoader.Load(items, base_dataloadoption);
			results.data = ObjectMapper.Map<IEnumerable<Street>, IEnumerable<StreetDto>>(results.data.Cast<Street>());
			return results;
		}
		...
		public virtual async Task<int> InsertFromExcelAsync(IFormFile file)
		{
			...
			await _streetRepository.GetQueryableAsync(); // to be remove
