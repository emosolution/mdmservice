...
namespace DMSpro.OMS.MdmService.WeightMeasurements
{
	public partial class WeightMeasurementsAppService
	{
		...
		{
			var items = await _weightMeasurementRepository.GetQueryableAsync();
			var base_dataloadoption = new DataSourceLoadOptionsBase();
			DataLoadParser.Parse(base_dataloadoption,inputDev);
			LoadResult results = DataSourceLoader.Load(items, base_dataloadoption);
			results.data = ObjectMapper.Map<IEnumerable<WeightMeasurement>, IEnumerable<WeightMeasurementDto>>(results.data.Cast<WeightMeasurement>());
			return results;
		}
		...
		public virtual async Task<int> InsertFromExcelAsync(IFormFile file)
		{
			...
			await _weightMeasurementRepository.GetQueryableAsync(); // to be remove
